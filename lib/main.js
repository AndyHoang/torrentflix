(function() {
    ...
    function writeHistory(torrent_name) {
        var history_array = get_hist_object.get('recentlyWatched.items');
        if (history_array.indexOf(torrent_name) == -1) {
          history_array.push(torrent_name);
          get_hist_object.set('recentlyWatched.items', history_array);
        }
    }
    ...
    function printHistory(){
      ...
      if(history === "true"){
        var all_hist = get_hist_object.get('recentlyWatched');

        if(all_hist.items.length > 0){
          console.log(chalk.green("Watched History: "));
          Object.keys(all_hist.items).forEach(function(key) {
              console.log(chalk.red(histcount + ' ') + chalk.magenta(all_hist.items[key]));
              histcount++;
          });
        } else {
    ...
    function searchHistory(search_title) {
      var all_hist = get_hist_object.get('recentlyWatched.items');
      if (all_hist.indexOf(search_title) > -1) {
        return true;
      } else {
        return false;
      }
    ...
    function streamTorrent_wo_sub_list(torrent, index){
      ...
      if(peerflix_player === "--airplay"){
        ...
        if (isWindows) {
          argsList = [torrent.split('&')[0], "--i=" + index, peerflix_player, peerflix_player_arg, peerflix_port];
        } else {
    ...
    function streamTorrent_sub_list(torrent, index){
      ...
      if(peerflix_player === "--airplay"){
        ...
        if (isWindows) {
          argsList = [torrent.split('&')[0], "--subtitles=\"" + peerflix_subtitle + "\"", "--i=" + index, peerflix_player, peerflix_player_arg, peerflix_port];
        } else {
          argsList = [torrent, "--subtitles=" + peerflix_subtitle, "--i=" + index, peerflix_player];
        }
      }
      osSpecificSpawn(peerflix_command, argsList);
    }
    ...
    function streamTorrent_wosub(torrent){
      ...
      if(peerflix_player === "--airplay"){
        ...
        if (isWindows) {
          argsList =  [torrent.split('&')[0], peerflix_player];
        } else{
          argsList =  [torrent, peerflix_player];
        }
      }
    ...
    function streamTorrent_sub(torrent){
      ...
      if(peerflix_player === "--airplay"){
        ...
        if (isWindows) {
          argsList = [torrent.split('&')[0], "--subtitles=\"" + peerflix_subtitle + "\"", peerflix_playe];
        } else {
          argsList = [torrent, "--subtitles=" + peerflix_subtitle, peerflix_player];
        }
      }
      osSpecificSpawn(peerflix_command, argsList);
    }
